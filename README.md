...
- v1.8a 修复：微信阈值告警正文中的申购/赎回状态优先使用 AkShare `fund_purchase_em` 批量接口；接口缺失、超时或单只基金未命中时，自动回退原东方财富 F10/基金页状态解析。
- v1.8a 调整：微信定时推送到点后不再等待后台数据刷新任务释放锁；推送使用已保存实时数据叠加快速 AkShare 快照，确保数据刷新不影响微信定时推送。
- v1.8a 明确：开盘/交易时段每 5 分钟刷新基金数据，休市/非交易时段每 30 分钟刷新；该刷新节奏与微信推送时间相互独立。
- v1.8a 修复：AkShare/EastMoney 行情接口增加 `push2`、`push2delay`、`88.push2`、`2.push2` 多主机兜底，并对断连、超时、空响应进行重试，降低 `ServerDisconnectedError` 导致整批 LOF 快照不可用的概率。
- v1.8a 增强：GitHub Actions/服务日志会输出每只基金的 `Source=[...]`，明确 NAV、估值、价格和折溢价来自 AkShare 还是原有接口/计算兜底。
...
- v1.8a 微信推送只用于折溢价阈值告警，不再发送定时汇总或测试消息；支持多个 Server酱 SendKey 同时推送。
